...
float particleX, particleZ;

int currentlevel = 1;

static float angle=0.0, ratio;
static float x = 0.0f, y = 1.0f, z = 5.0f;		//camera coords
...
// Position of Light0
GLfloat sphereLightPos[] = {spherePosX, spherePosY + 5, spherePosZ, 1};
//GLfloat sphereLightPos[] = {spherePosX, spherePosY + SPHERE_RAD/2.0, spherePosZ, 1.0f};
float lightX = -120; float lightY = 5; float lightZ = 4;
GLfloat light0Pos[] = {lightX, lightY, lightZ};

GLuint grassTexture;
GLuint skyboxTexture;
GLuint sphereTexture;
GLuint brickTexture;
GLuint brickZTexture;
GLuint roofTexture;
GLuint redTexture;
GLuint treeTexture;
GLuint introScreen;
...
void initScene() {
	...
	brickZTexture = LoadTexture( "textures/brickZ.raw", 1024, 1024 );
	glBindTexture( GL_TEXTURE_2D, brickZTexture);
	...
	introScreen = LoadTexture( "textures/title15.raw", 640, 360);
...
void moveMeFlat(float i) {		//moving forward/back by i units
	...
	if (playAgainMode == 0) {
		if (collisions->checkCollision(newPosX, newPosY, newPosZ, SPHERE_RAD)) {	//check if there are obstacles in the intended location
			...
			if ((spherePosX > particleX - 5) && (spherePosX < particleX +5) && (spherePosZ > particleZ - 5) && (spherePosZ < particleZ + 5)) {
				// Restore health if the sphere enters a fountain
				zen = 100;
			}
...
void renderScene(void) {
	...
	// Light0 follows the sphere
	glEnable(GL_LIGHT0);
	glLightfv(GL_LIGHT0, GL_POSITION, sphereLightPos);
	//glLightfv(GL_LIGHT0, GL_POSITION, light0Pos);
		...
		// Figure out where to draw the fountains
		if (currentlevel == 1) {
			particleX = 138;
			particleZ = -176;
		...
		glBindTexture(GL_TEXTURE_2D, brickTexture);
		drawing->drawFountain(particleX, particleZ);
		// Draw a fountain
		drawParticleSystem(3, particleX, particleZ);
		cnt++;
		if(cnt % 3 == 0) {
			nextFrameParticleSystem();
			insertParticles(particleX, particleZ);
			cnt = 0;
		}
		

		if (moveCount < 30) {
			//display the intro/instructions until the user moves a little
		...
		// We don't want any HUD stuff to get lit, only the 3D stuff should be lit. Turn off lighting now that we're done drawing 3D stuff.
		glDisable(GL_LIGHTING);
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	if ((key == 27) || (key == 'q') || (key == 'Q')) {
		...
		printf("%f, %f", spherePosX, spherePosZ);
	}
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("SphereQuest 1.5");
